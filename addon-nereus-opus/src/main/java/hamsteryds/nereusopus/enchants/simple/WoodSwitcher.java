...
public class WoodSwitcher extends EventExecutor {
   ...
   public void blockBreak(int level, BlockBreakEvent event) {
      if (this.targets.contains(event.getBlock().getType())) {
         ...
         FoliaUtils.runTask(
            event.getBlock(),
            t -> event.getBlock().getWorld().dropItem(event.getBlock().getLocation(), new ItemStack(this.drops.get((int)(this.drops.size() * Math.random()))))
         );
      }
   }
}
